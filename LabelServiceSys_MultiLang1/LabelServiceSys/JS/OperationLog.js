$(function () {
    var _$_datagrid = $("#DG_Log");
    var _$_ddlOpType = $("#ddlOpType");
    var QueryURL = "/OperationLog/GetData";
    var DeleteURL = "/OperationLog/Delete";
    var QueryOperationLogTypeURL = "/Default/LoadOperationLogTypeJSON";

    _$_ddlOpType.combobox({
        url: QueryOperationLogTypeURL,
        valueField: 'id',
        textField: 'text',
        editable: false,
        panelHeight: null,
        onLoadSuccess: function () {
            _$_ddlOpType.combobox("setValue", "-99");
        }
    });

    var PrintURL = "";
    QueryURL = "/OperationLog/GetData?dBegin=" + encodeURI($("#txtBeginD").val()) + "&dEnd=" + encodeURI($("#txtEndD").val()) + "&opType=" + encodeURI(_$_ddlOpType.combobox("getValue")) + "&urName=" + encodeURI($("#txturName").val());
    ...
    _$_datagrid.datagrid({
        ...
        columns: [[
                    ...
                    { field: 'opJobNameIdLv1', title: '一级工作流程', width: 80, sortable: true,
                        sorter: function (a, b) {
                            return (a > b ? 1 : -1);
                        }
                    },
                     { field: 'opJobNameIdLv2', title: '二级工作流程', width: 80, sortable: true,
                         sorter: function (a, b) {
                             return (a > b ? 1 : -1);
                         }
                     },
                    { field: 'op_urUnitCode', title: '口岸代码', width: 80, sortable: true,
                        sorter: function (a, b) {
                            return (a > b ? 1 : -1);
                        }
                    },
                    { field: 'opDateTime', title: '操作时间', width: 130, sortable: true,
                        sorter: function (a, b) {
                            return (a > b ? 1 : -1);
                        }
                    },
                    { field: 'opContent', title: '操作内容', width: 300, sortable: true,
                        sorter: function (a, b) {
                            return (a > b ? 1 : -1);
                        }
                    },
                    { field: 'opTypeDesc', title: '日志类型', width: 100, sortable: true,
                        sorter: function (a, b) {
                            return (a > b ? 1 : -1);
                        }
                    }
    ...
    function Print() {
        PrintURL = "/OperationLog/Print?order=" + _$_datagrid.datagrid("options").sortOrder + "&sort=" + _$_datagrid.datagrid("options").sortName + "&dBegin=" + encodeURI($("#txtBeginD").val()) + "&dEnd=" + encodeURI($("#txtEndD").val()) + "&opType=" + encodeURI(_$_ddlOpType.combobox("getValue")) + "&urName=" + encodeURI($("#txturName").val()) + "&page=1&rows=10000000";
    ...
    function Excel() {
        ...
        PrintURL = "/OperationLog/Excel?order=" + _$_datagrid.datagrid("options").sortOrder + "&sort=" + _$_datagrid.datagrid("options").sortName + "&dBegin=" + encodeURI($("#txtBeginD").val()) + "&dEnd=" + encodeURI($("#txtEndD").val()) + "&opType=" + encodeURI(_$_ddlOpType.combobox("getValue")) + "&urName=" + encodeURI($("#txturName").val()) + "&page=1&rows=10000000&browserType=" + browserType;
        if (_$_datagrid.datagrid("getData").rows.length > 0) {
